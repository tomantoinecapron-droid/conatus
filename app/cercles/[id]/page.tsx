...
export default function CerclePage() {
  ...
  const loadPage = async (userId: string) => {
    setPageError('')
    try {
      // Charger cercle + membres en parallèle
      ...
      // Vérifier accès
      const isMember = membersData.some((m: any) => m.user_id === userId)
      ...
      setIsOwner(circleData.owner_id === userId)
    ...
    <div className="min-h-screen bg-[#1a1714] text-white pb-24">
          ...
          {feed.length === 0 ? (
            ...
            <div className="bg-[#242018] border border-white/8 rounded-2xl overflow-hidden">
              {feed.map((item, i) => (
                <div
                  key={item.id}
                  className={`flex items-center gap-3 px-4 py-3 ${i < feed.length - 1 ? 'border-b border-white/5' : ''}`}
                >
      ...
      {tab === 'membres' && (
        <div className="px-5 pt-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {members.map(member => {
              ...
              return (
                <div
                  key={member.user_id}
                  className="flex items-center gap-3 bg-[#242018] border border-white/8 rounded-xl px-4 py-3"
                >
                  ...
                  <a href={`/profil/${p?.username}`} className="flex-1 min-w-0">
                    {p?.full_name && (
                      <p className="font-medium text-white text-sm truncate">{p.full_name}</p>
                    )}
                  ...
                  {member.role === 'owner' ? (
                    ...
                    <button
                      onClick={() => handleRemoveMember(member.user_id, p?.username)}
          ...
          {/* Section invitation (owner seulement) */}
          {isOwner && (
            <div className="bg-[#242018] border border-white/8 rounded-2xl p-4">
              ...
              {inviteSuccess && (
                <p className="text-emerald-400 text-xs mt-2">{inviteSuccess}</p>
              )}

              {/* Résultats */}
              {inviteLoading && (
                <p className="text-[#7a7268] text-xs mt-3 text-center">Recherche...</p>
              )}
              ...
              {inviteResults.length > 0 && (
                <div className="flex flex-col gap-2 mt-3">
                  {inviteResults.map(profile => (
                    <div
                      key={profile.id}
                      className="flex items-center gap-3 bg-[#1a1714] border border-white/8 rounded-xl px-3 py-2.5"
                    >
                      ...
                      <button
                        ...
                        className="text-[#c9440e] text-xs font-medium hover:opacity-80 transition shrink-0"
                      >
                        Ajouter
                      </button>
                    </div>
